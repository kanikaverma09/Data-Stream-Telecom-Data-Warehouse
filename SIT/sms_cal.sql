...
CURSOR FOR
  ...
  FETCH calc_cursor into pv_subscriber_id,pv_plan_type,pv_sms_used,
	...
	-- IF pv_sms_used <> 0 then
  	SET pv_count=(select count(subscriber_id) FROM crtest_Slipstream_Telecom.cr_bill 
  	...
  	IF  pv_count <> 0  then
     	...
     	ELSE
    		...
    		INSERT INTO crtest_Slipstream_Telecom.cr_bill(subscriber_id,sms_bill) values(pv_subscriber_id,pv_sms_bill);
 	END IF; 
 		FETCH calc_cursor into pv_subscriber_id,pv_plan_type,pv_sms_used,
            		pv_free_sms,pv_sms_rate; 
  	END WHILE;
