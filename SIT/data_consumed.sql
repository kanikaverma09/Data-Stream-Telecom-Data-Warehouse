CREATE PROCEDURE dataConsumedtest_cal()
BEGIN
	...
	DECLARE calc_cursor CURSOR FOR
		...
		from crtest_Slipstream_Telecom.cr_transaction t , crtest_Slipstream_Telecom.cr_plan_master p
	...
	OPEN calc_cursor;
		...
		WHILE(SQLCODE=0) DO
 			IF pv_data_consumed <> 0 then
 				SET pv_count=(select count(subscriber_id) FROM crtest_slipstream_telecom.cr_bill where subscriber_id=pv_subscriber_id);
 				IF  pv_count <> 0 then
					UPDATE crtest_slipstream_telecom.cr_bill
  				...
  				ELSE
  					...
  					INSERT INTO crtest_slipstream_telecom.cr_bill(subscriber_id,data_bill) values(pv_subscriber_id,pv_data_bill);
  				END IF;
 			END IF; 
